...
int main(void) {
    ...
    unsigned char* host_buf =
        mmap(NULL, BUF_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, memfd, 0);
    ...
    pbox_call3(box, fill_fn, int, PBOX_TYPE_VOID, void*, PBOX_TYPE_POINTER,
               sandbox_buf, size_t, PBOX_TYPE_UINT64, (size_t) BUF_SIZE,
    ...
    pbox_call2(box, inc_fn, int, PBOX_TYPE_VOID, void*, PBOX_TYPE_POINTER,
               sandbox_buf, size_t, PBOX_TYPE_UINT64, (size_t) BUF_SIZE);
    ...
    // Read it back via the sandbox (call a function that returns the first
    // byte) For simplicity, we'll just show that the sandbox can see it by
