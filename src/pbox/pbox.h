...
// Generic function pointer type for type-erased callback storage
typedef void (*pbox_fn_t)(void);

// Callback dispatch function type.
// Called on the host when the sandbox invokes a callback.
// Unpacks arguments from arg_storage using arg_offsets and calls func_ptr.
typedef void (*pbox_callback_dispatch_fn)(pbox_fn_t func_ptr,
                                          const char* arg_storage,
                                          const uint64_t* arg_offsets,
                                          char* result_storage);

// Register a host function as a callback.
// dispatch: typed dispatcher generated by the C++ wrapper.
